...
export default class ProposalComponentsPage {
  ...
  getTitle() {
    return element(by.id('proposal-prop-heading'));
  }
  ...
  async waitUntilProposalCountIs(length) {
    await waitUntilCount(element.all(by.css('div table .btn-danger')), length);
  }
}

export class ProposalDeleteDialog {
  private dialogTitle: ElementFinder = element(by.id('proposalApp.proposal.delete.question'));
