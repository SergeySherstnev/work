...
export default class CommentComponentsPage {
  ...
  getTitle() {
    return element(by.id('comment-prop-heading'));
  }
  ...
  async waitUntilCommentCountIs(length) {
    await waitUntilCount(element.all(by.css('div table .btn-danger')), length);
  }
}

export class CommentDeleteDialog {
  private dialogTitle: ElementFinder = element(by.id('proposalApp.comment.delete.question'));
